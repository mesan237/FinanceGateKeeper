...
| VS-17: Expense Edit & Delete  | 🔲 Backlog | updateExpense + deleteExpense in service, useExpenseEdit hook, ExpenseDetailScreen (pre-filled form + delete with confirmation), ExpenseDetailRoute, app/expenses/[id].tsx route, tappable expense rows in TransactionList. Income rows remain non-tappable (income edit deferred). Blocked by VS-16. |
